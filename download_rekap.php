<?php
        ...
        while ($rowmanager = mysqli_fetch_row($show_manager)) {
            $nama_manager = $rowmanager[0];
                            ...
                            $strhtml.= "<br/>";
                            $strhtml.= "PT PLN (PERSERO) UNIT INDUK WILAYAH RIAU DAN KEPRI $nama_area";
                            $strhtml.= "<br/>";
                            $strhtml.= "$nama_vendor";
                            $strhtml.="</div>";
                            $strhtml.= "<br/>";
                            $strhtml.= "<table class='hei'>
									<thead><tr><th>Nomor</th><th>Nomor SPBJ</th><th>Paket Jenis</th><th>MUTU</th><th>SDM DAN KEUANGAN</th><th>LINGKUNGAN,K3&APD</th>
												<th>ADMINISTRASI</th><th>KOMUNIKASI DAN RESPONSIVENESS</th><th>JUMLAH</th>
												<th>KETERANGAN</th></tr></thead><tbody>";
                                    
                            /*$query = mysqli_query($mysqli, $connection,"
                            ...
                            $query = mysqli_query($mysqli, $connection, "
									SELECT B.spj_no, SUM(case when B.id_deskripsi = 1 then ((b.nilai*b.bobot)/10) end)MUTU, 
										...
										SUM(case when B.id_deskripsi = 6 then ((b.nilai*b.bobot)/10) end)Administrasi, 
										SUM(case when B.id_deskripsi = 7 then ((b.nilai*b.bobot)/10) end)Komunikasi,
										c.PAKET_JENIS FROM penilaian_nilai B join tb_spj C JOIN tb_termin D where b.spj_no=C.SPJ_NO and C.SPJ_NO=D.spj_no and C.VENDOR_ID=$vendor_id
										and C.AREA_KODE=$print_area AND D.verifikasi_mup3=1 GROUP BY B.spj_no ORDER by c.PAKET_JENIS				
							");
                    ...
                    $strhtml.="<br/>";
                    $strhtml.= "<div class='print_manager'>$kota_area, $today";
                    $strhtml.="<br/>";
