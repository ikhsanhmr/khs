<?php
	...
	while ($rows=mysqli_fetch_row($result)){ 
		...
		if($data1[0][0]<=$data2[0][0]){
			...
			$quer = mysqli_query("UPDATE TB_FIN_VENDOR A, TB_SPJ B
								SET A.FIN_CURRENT = A.FIN_CURRENT - $get_var_nominal_tagihan 
								WHERE A.VENDOR_ID = B.VENDOR_ID 
								AND B.SPJ_NO = '$get_var_no_spj'");
			
			$quer2 = mysqli_query("UPDATE TB_PAGU_KONTRAK A
			...
			$resultQuery_keterangan=mysqli_query($query_keterangan_bayar);
