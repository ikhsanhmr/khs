...
$(document).ready(function() {
    ...
    <body class="skin-black">
        ...
        <div class="row">
            <div class="col-md-12">
                <?php if ($print_session==0 or $print_session==1 or $print_session==12) {?>
             <section class="panel">
    ...
    <div class="row">
		<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">tabel kecepatan kerja</header>
					...
					<tbody>
						<?php
                            ...
                            if ($area=="" && $vendor=="") {
								...
								(select coalesce(max(d.progress_value)) from tb_progress d where d.spj_no = a.spj_no )
								, a.spj_deskripsi
								, a.spj_target
								, c.satuan
								, (select coalesce(max(d.realisasi),0) from tb_progress d where d.spj_no = a.spj_no )
								,a.SPJ_NILAI, a.SPJ_TANGGAL_AKHIR, a.SPJ_ADD_NILAI
								,(select coalesce(max(d.NO_TUG9)) from tb_progress d where d.spj_no=a.spj_no)
								,(select coalesce(max(d.TGL_TUG9)) from tb_progress d where d.spj_no=a.spj_no)
								,(select coalesce(max(d.NO_TUG10)) from tb_progress d where d.spj_no=a.spj_no)
								,(select coalesce(max(d.TGL_TUG10)) from tb_progress d where d.spj_no=a.spj_no)
								FROM tb_spj a left join tb_vendor b on a.VENDOR_ID = b.VENDOR_ID
								LEFT JOIN tb_paket c on a.PAKET_JENIS = c.PAKET_JENIS
                                ...
                                LEFT JOIN tb_skko_i e on a.SKKI_NO = e.SKKI_NO
                                LEFT JOIN tb_area f on e.AREA_KODE = f.AREA_KODE
                                WHERE f.AREA_KODE = $area  and a.vendor_id != 106";
                            } elseif ($area=="" && $vendor!="") {
                                $count = "SELECT DISTINCT count(a.spj_no)
									FROM tb_spj a left join tb_vendor b on a.VENDOR_ID = b.VENDOR_ID
									LEFT JOIN tb_paket c on a.PAKET_JENIS = c.PAKET_JENIS
                                    LEFT JOIN tb_skko_i e on a.SKKI_NO = e.SKKI_NO
                                    LEFT JOIN tb_area f on e.AREA_KODE = f.AREA_KODE
                                    WHERE b.VENDOR_NAMA LIKE '%$vendor%'  and a.vendor_id != 106";
                                $count_res = mysqli_query($mysqli, $count);
                                $r = mysqli_fetch_row($count_res);
                                $numrows = $r[0];
                                ...
                                $count = "SELECT DISTINCT count(a.spj_no)
                                    ...
                                    LEFT JOIN tb_skko_i e on a.SKKI_NO = e.SKKI_NO
                                    LEFT JOIN tb_area f on e.AREA_KODE = f.AREA_KODE
                                    WHERE f.AREA_KODE = $area and b.VENDOR_NAMA LIKE '%$vendor%'  and a.vendor_id != 106";
                                $count_res = mysqli_query($mysqli, $count);
                                $r = mysqli_fetch_row($count_res);
                                $numrows = $r[0];
                                ...
                                for ($i=0;$i<count($data);$i++) {
                                    ...
                                    $current_desc = $data[$i][6];
                                    $current_target = $data[$i][7];
                                    $coma_current_target=number_format($current_target, 2);
                                    $titik_to_coma_convert = str_replace(".", ",", $coma_current_target);
                                    $current_satuan = $data[$i][8];
                                    $current_realisasi = $data[$i][9];
                                    $current_nilai = $data[$i][10];

                                    $current_tgl_akhir = $data[$i][11];
                                    $tgl_akhir = date("d-m-Y", strtotime($current_tgl_akhir));

                                    $current_nilai_add = $data[$i][12];
                                    $no_tug9=$data[$i][13];
                                    if ($no_tug9===null) {
                                        $print_tug9='belum update';
                                    } else {
                                        $print_tug9=$no_tug9;
                                    }
                                    $tgl_tug9=$data[$i][14];
                                    if ($tgl_tug9===null) {
                                        $print_tgl_tug9='belum update';
                                    } else {
                                        $print_tgl_tug9=$tgl_tug9;
                                    }
                                    $no_tug10=$data[$i][15];
                                    if ($no_tug10===null) {
                                        $print_no_tug10='belum update';
                                    } else {
                                        $print_no_tug10=$no_tug10;
                                    }
                                    $tgl_tug10=$data[$i][16];
                                    if ($tgl_tug10===null) {
                                        $print_tgl_tug10='belum update';
                                    } else {
                                        $print_tgl_tug10=$tgl_tug10;
                                    }
                                    if ($b>100) {
                                        $b = 100;
                                    }
                                    ...
                                    echo "<tr><td>$a</td>
										  ...
										  <td>$current_nilai</td>
										  <td>$current_nilai_add</td>
										  <td>$tgl_akhir</td>
										  <td>AREA $current_nama_area</td>
										  <td>$current_nama_vendor</td>
										  <td>$current_jenis_pekerjaan</td>
										  <td>$current_desc</td></td>
										  <td><span class='$class1'>$current_progress_pekerjaan %</span></td>
										  <td>$titik_to_coma_convert $current_satuan</td>
										  <td>$current_realisasi $current_satuan</td>
										  <td>$print_tug9</td>
										  <td>$print_tgl_tug9</td>
										  <td>$print_no_tug10</td>
										  <td>$print_tgl_tug10</td></tr>
										  ";
                                }
                        ?>
					</tbody>
