...
$spj = mysqli_query($mysqli, "SELECT
...
$select_vendor = mysqli_query($mysqli, "SELECT vendor_id, paket_jenis, spj_add_nilai FROM tb_spj where SPJ_NO = '$get_var_no_spj'");
while ($rows2=mysqli_fetch_array($select_vendor)) {
    ...
    $query2 = mysqli_query($mysqli, "select pagu_kontrak, terpakai from tb_pagu_kontrak where vendor_id = '$vendor_id' and paket_jenis = '$paket_jenis'");
...
//query finansial vendor
    $select_finansial_vendor = mysqli_query($mysqli, "SELECT fin_limit, fin_current FROM tb_fin_vendor where vendor_id = '$vendor_id'");
    ...
    /*$skki = mysqli_query($mysqli, "SELECT b.skki_nilai FROM tb_spj a, tb_skko_i b WHERE a.skki_no = b.skki_no AND a.spj_no = '$get_var_no_spj'");
    while($rows2=mysqli_fetch_array($skki)){
        $res2[]=$rows2;
    }*/
    
    $skki_terpakai = mysqli_query($mysqli, "SELECT skki_terpakai, skki_nilai FROM tb_skko_i WHERE skki_no = '$get_var_skki_tujuan'");
    while ($rows=mysqli_fetch_array($skki_terpakai)) {
        $skki[]=$rows;
    }
     
        
    $tgl_akhir = mysqli_query($mysqli, "SELECT 
...
} elseif ($get_var_no_addendum=="") {
    echo '<script language="javascript">alert("nomor addendum harus di isi")</script>';
    echo '<script language="javascript">window.location = "inp_addendum.php"</script>';
} elseif ($get_var_nilai_addendum==0 && $get_var_tanggal_akhir=="" && $get_var_skki_tujuan=="-" && $get_ad_var_target=="") {
...
} elseif ((($skki[0][0]+$get_var_nilai_addendum) > $skki[0][1]) and ($get_var_skki_tujuan != '-')) {
...
//buka bentar untuk buka adendum dengan tanggal adendum mundur, request UPK
    ...
    //echo '<script language="javascript">window.location = "inp_addendum.php"</script>';
} elseif (($current - $spj_nilai_sebelum_adendum + $get_var_nilai_addendum)>=$pagu_kontrak) {
    echo '<script language="javascript">alert("nilai adendum spj lebih dari nilai pagu kontrak")</script>';
    echo '<script language="javascript">window.location = "inp_addendum.php"</script>';
} elseif (($fin_current - $spj_nilai_sebelum_adendum + $get_var_nilai_addendum)>=$fin_limit) {
    ...
    $res_insert_tb_add = mysqli_query($mysqli, $insert_tb_add);

    if ($get_var_tanggal_akhir=="" && $get_var_nilai_addendum!=0 && $get_ad_var_target=="") {
        //$res_insert_tb_add = mysqli_query($mysqli, $insert_tb_add);
        $res_update_fin_vendor = mysqli_query($mysqli, $update_fin_vendor);
        $res_update_skki_terpakai = mysqli_query($mysqli, $update_skki_terpakai);
        $res_update_nilai_add = mysqli_query($mysqli, $update_nilai_add);
        $res_update_tanggal_akhir = 1;
        $res_update_target_volume = 1;//ditambah 27 juli 2018
        $res_update_pagu_terpakai = mysqli_query($mysqli, $update_pagu_terpakai);
    } elseif ($get_var_tanggal_akhir!="" && $get_var_nilai_addendum==0 && $get_ad_var_target!="") {
        //$res_insert_tb_add = mysqli_query($mysqli, $insert_tb_add);;
        $res_update_fin_vendor = 1;
        $res_update_skki_terpakai = 1;
        $res_update_nilai_add = 1;
        $res_update_tanggal_akhir = mysqli_query($mysqli, $update_tanggal_akhir);
        $res_update_tanggal_awal = mysqli_query($mysqli, $update_tanggal_addendum);
        $res_update_target_volume = mysqli_query($mysqli, $update_adendum_target);
    ...
    if ($get_var_skki_tujuan != '-') {
        $GET_NILAI_ADD_DB =select_nilai_adds($get_var_no_spj, $mysqli);
        ...
        mysqli_query($mysqli, $update_nilai_skki_awal);
        mysqli_query($mysqli, $update_nilai_skki_tujuan);
        mysqli_query($mysqli, $update_skki_spj);
        $res_update_fin_vendor = 1;
        $res_update_skki_terpakai = 1;
        $res_update_nilai_add = 1;
        $res_update_tanggal_akhir = 1;
