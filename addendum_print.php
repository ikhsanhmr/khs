...
function format_indo($date)
{
    ...
    $getdata_queryr = mysqli_query($qr);
...
*/
    ...
    $dir_vendor     = get_direksi($spj_data[0][VENDOR_ID]);
    ...
    $no_spp     = get_no_spp($vendor, $paket_kerja);
    $tgl_spp_f  = date('Y-m-d',strtotime(get_tgl_spp($vendor, $paket_kerja)));
    $tgl_spp    = format_indo($tgl_spp_f);

    $no_pen     = get_no_penawaran($vendor, $paket_kerja);
    $tgl_pen_f  = date('Y-m-d',strtotime(get_tgl_penawaran($vendor, $paket_kerja)));
    $tgl_pen    = format_indo($tgl_pen_f);
    //echo $tgl_pen;

    $no_rks     = get_no_rks($vendor, $paket_kerja);
    $tgl_rks_f  = date('Y-m-d',strtotime(get_tgl_rks($vendor, $paket_kerja)));
    ...
    $q = "select vendor_nama,paket_deskripsi2
        ...
        $jabatan_vendor = get_jabatan_vend($vendor);
