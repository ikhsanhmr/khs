<?php
	...
	/*if($get_var_paket_pekerjaan==1){
		$check = mysqli_query("select a.vendor_id from tb_fin_vendor a,tb_mapping_vendor b where a.vendor_id = b.vendor_id and a.vendor_id=$get_var_vendor and b.area_kode=$area_kode and b.paket_jenis=$get_var_paket_pekerjaan");
	...
	/*$fin_current = mysqli_query("SELECT fin_limit, fin_current FROM tb_fin_vendor WHERE vendor_id = $get_var_vendor");
	while($rows=mysqli_fetch_array($fin_current)){
								$data[]=$rows;
							}*/
	$skki_terpakai = mysqli_query("SELECT skki_terpakai, skki_nilai FROM tb_skko_i WHERE skki_no = '$get_var_no_skkio'");
	...
	if($get_var_no_skkio==""){
		...
		$vendorInsert=mysqli_query($query_vendor);
