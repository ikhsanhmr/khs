<?php
	...
	$q = mysqli_query("SELECT spj_no,spj_nilai,spj_add_nilai,vendor_id,skki_no,PAKET_JENIS FROM tb_spj WHERE spj_no = '$get_var_spj_no'");
	...
	$fin_curr_Q = mysqli_query("SELECT fin_limit, fin_current FROM tb_fin_vendor WHERE vendor_id = $vendor_id");
	...
	$query2a = mysqli_query("select pagu_kontrak, terpakai from tb_pagu_kontrak where vendor_id = '$vendor_id' and paket_jenis = '$paket_jenis'");
	...
	$skki_Q = mysqli_query("SELECT skki_nilai,skki_terpakai FROM tb_skko_i WHERE skki_no = '$skki_no'");
	...
	$jlh_pembayaran_Q = mysqli_query("SELECT COALESCE(sum(pembayaran_nominal),0) FROM tb_pembayaran WHERE spj_no = '$get_var_spj_no'");
	while($rows2=mysqli_fetch_row($jlh_pembayaran_Q)){
		$data2[]=$rows2;
	}
	$ttl_pembayaran = $data2[0][0];
	
	$add_exist = mysqli_query("SELECT * FROM TB_ADDENDUM WHERE SPJ_NO = '$get_var_spj_no'");
	...
	if($isexist != null){
		...
		$update_add = mysqli_query("UPDATE TB_ADDENDUM SET addendum_nilai = $get_var_spj_nilai WHERE spj_no = '$get_var_spj_no' AND addendum_nilai = $add_nilai");
		...
		$update_add = mysqli_query("UPDATE tb_spj SET spj_add_nilai = $get_var_spj_nilai WHERE spj_no = '$get_var_spj_no'");
		$diff = $get_var_spj_nilai - $data[0][1];
	}
	
	$query = mysqli_query("UPDATE tb_spj SET $edit_variable = $get_var_spj_nilai WHERE SPJ_NO = '$get_var_spj_no'") or die("MySQL error:".mysqli_error());
	$query2 = mysqli_query("UPDATE tb_fin_vendor SET fin_current = fin_current + $diff WHERE vendor_id = $vendor_id");
	$query3 = mysqli_query("UPDATE tb_skko_i SET skki_terpakai = skki_terpakai + $diff WHERE skki_no = '$skki_no'");
	$query4 = mysqli_query("UPDATE tb_pagu_kontrak SET terpakai = terpakai + $diff WHERE vendor_id = $vendor_id and PAKET_JENIS = (SELECT PAKET_JENIS from tb_spj where SPJ_NO = '$get_var_spj_no')");
	$query5 = mysqli_query("UPDATE tb_spj SET PPN = $get_var_ppn, MIN_PPN = $get_var_min_ppn WHERE SPJ_NO = '$get_var_spj_no'") or die("MySQL error:".mysqli_error());
