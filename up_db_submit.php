													...
													if($arrayCount>1){
														if($jenis_data==1){
															...
															for($i=2;$i<=$arrayCount;$i++){
																...
																$spj_exist_query = mysqli_fetch_array(mysqli_query("SELECT spj_no FROM ".$databasetable." WHERE spj_no = '".$spj_no[$i-2]."' and vendor_id = ".$vendor_id[$i-2]." and skki_no = '".$skki_no[$i-2]."' and paket_jenis = ".$paket_jenis[$i-2]." and spj_nilai = ".$spj_nilai[$i-2]." and spj_tanggal_mulai = '".$tgl_mulai_db[$i-2]."' and spj_tanggal_akhir = '".$tgl_akhir_db[$i-2]."' and spj_add_nilai = ".$spj_add_nilai[$i-2]." and spj_add_tanggal = '".$spj_add_tanggal[$i-2]."'  and spj_input_user = '".$spj_input_user[$i-2]."'"));
																$spj_exist = $spj_exist_query["spj_no"];
																
																//	PRIMARY KEY VALIDATION	//
																$spj_primary_query = mysqli_fetch_array(mysqli_query("SELECT spj_no FROM tb_spj WHERE spj_no ='".$spj_no[$i-2]."'"));
																$spj_primary = $spj_primary_query["spj_no"];
																
																//	VENDOR ID VALIDATION//
																$spj_vendor_id_query = mysqli_fetch_array(mysqli_query("SELECT vendor_id FROM tb_vendor WHERE vendor_id = ".$vendor_id[$i-2]));
																$spj_vendor_id = $spj_vendor_id_query["vendor_id"];
																
																//	SKKO/I NUMBER VALIDATION	//
																$skkoi_no_query = mysqli_fetch_array(mysqli_query("SELECT skki_no, skki_nilai, skki_terpakai FROM tb_skko_i WHERE skki_no ='".$skki_no[$i-2]."'"));
																$skkoi_no = $skkoi_no_query["skki_no"];
																$skkoi_sisa = $skkoi_no_query["skki_nilai"] - $skkoi_no_query["skki_terpakai"];

																// SKKO/I AREA VALIDATION //
																$skkoi_area_query = mysqli_fetch_array(mysqli_query("select vendor_id from tb_mapping_vendor a, tb_skko_i b
																...
																$spj_paket_jenis_vendor_id_query = mysqli_fetch_array(mysqli_query("SELECT vendor_id FROM tb_mapping_vendor WHERE vendor_id = ".$vendor_id[$i-2]." AND paket_jenis = ".$paket_jenis[$i-2].""));
																$spj_paket_jenis_vendor_id = $spj_paket_jenis_vendor_id_query["vendor_id"];
																
																//	FINANCIAL LIMIT VALIDATION	//
																$spj_fin_lim_query = mysqli_fetch_array(mysqli_query("SELECT fin_limit, fin_current FROM tb_fin_vendor WHERE vendor_id = ".$vendor_id[$i-2]));
																$spj_sisa_fin = $spj_fin_lim_query["fin_limit"] - $spj_fin_lim_query["fin_current"];
																
																//	SPJ INPUT USER VALIDATION	//
																$spj_inp_user_query = mysqli_fetch_array(mysqli_query("SELECT username FROM tb_user WHERE username = '".$spj_input_user[$i-2]."'"));
																...
																if($spj_exist!="") {
																	...
																	$insertTable_spj= mysqli_query("insert into ".$databasetable." (
																	SPJ_NO,
																	VENDOR_ID,
																	SKKI_NO,
																	PAKET_JENIS,
																	...
																	$update_tb_skkoi = mysqli_query("UPDATE tb_skko_i SET skki_terpakai = skki_terpakai+".$spj_nilai[$i-2]." WHERE skki_no = '".$skki_no[$i-2]."'");
																	$update_tb_fin_vendor = mysqli_query("UPDATE tb_fin_vendor SET fin_current = fin_current + ".$spj_nilai[$i-2]." WHERE vendor_id = ".$vendor_id[$i-2]);
															...
															echo "<a href='up_db.php' class='btn btn-info'>Back</a>
																...
																for($i=2;$i<=$arrayCount;$i++){
																	...
																	//	DATA EXIST VALIDATION	//
																	$skki_exist_query = mysqli_fetch_array(mysqli_query("SELECT skki_no FROM ".$databasetable." WHERE skki_jenis = '".$skki_jenis[$i-2]."' and skki_no = '".$skki_no[$i-2]."' and area_kode = '".$area_kode[$i-2]."' and skki_nilai = '".$skki_nilai[$i-2]."' and skki_terpakai = '".$skki_terpakai[$i-2]."' and skki_tanggal = '".$skki_tanggal[$i-2]."'"));
																	$skki_exist = $skki_exist_query["skki_no"];
																	
																	// PRIMARY KEY VALIDATION	//
																	$skki_primary_query = mysqli_fetch_array(mysqli_query("SELECT skki_no FROM ".$databasetable." WHERE skki_no = '".$skki_no[$i-2]."'"));
																	$skki_primary = $skki_primary_query["skki_no"];
																	
																	//	AREA KODE VALIDATION	//
																	$skki_area_kode_query = mysqli_fetch_array(mysqli_query("SELECT area_kode FROM tb_area WHERE area_kode=".$area_kode[$i-2]));
																	...
																	if($skki_exist!="") {
																		...
																		$skki_insertTable= mysqli_query("insert into ".$databasetable." values('".$skki_jenis[$i-2]."', '".$skki_no[$i-2]."', ".$area_kode[$i-2].", ".$skki_nilai[$i-2].", ".$skki_terpakai[$i-2].", '".$skki_tanggal_db[$i-2]."')");
																		$err[$i-2] = "<font color='green'>Record has been added</font>";
																	}
																}
																echo "<a href='up_db.php' class='btn btn-info'>Back</a>
