        ...
        while ($row_paket=mysqli_fetch_row($show_paket)) {
            $paket_deskripsi = $row_paket[0];
                            ...
                            $strhtml.= "<table class='hei'>
									<thead><tr><th>Nomor</th><th>Nama Perusahaan</th><th>MUTU</th><th>SDM DAN KEUANGAN</th><th>LINGKUNGAN,K3&APD</th>
                            ...
                            $query = mysqli_query($connection, "
										...
										SUM(case when B.id_deskripsi = 6 then ((b.nilai*b.bobot)/10) end)Administrasi,
										SUM(case when B.id_deskripsi = 7 then ((b.nilai*b.bobot)/10) end)Komunikasi 
										FROM penilaian_nilai B join tb_spj D JOIN tb_vendor C JOIN tb_termin E where B.spj_no=D.SPJ_NO and 
										D.VENDOR_ID=C.VENDOR_ID and D.SPJ_NO=E.spj_no AND D.AREA_KODE=$print_area AND D.PAKET_JENIS=$pilih_paket AND E.verifikasi_mup3=1
                    ...
                    $strhtml.="<br/>";
                    $strhtml.= "<div class='print_manager2'>$kota_area, $today";
                    $strhtml.="<br/>";
